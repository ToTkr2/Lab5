...
#define FILE_SIGN_AREA   ((L-1-K) / B + 1)   //保留区文件标识符起始块号（位图之后）
#define FILE_NUM FILE_BLOCK_LENGTH  //目录内最多文件数目 
#define BUFFER_LENGTH 25  //打开文件标目中的缓冲区长度
...
struct filesign{   //文件描述符 
	...
	int file_block_ary[FILE_BLOCK_LENGTH];   //文件分配块号数组 
}; 
